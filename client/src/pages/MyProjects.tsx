        ...
        {loading ? (
          ...
          <div className='py-10 min-h-[80vh]'>
            <div className='flex items-center justify-between mb-12'>
              ...
              <button onClick={() => navigate('/')} className='flex items-center gap-2 text-white px-3 sm:px-6 py-1 sm:py-2 rounded bg-indigo-600 hover:opacity-90 active:scale-95 transition-all'>
            ...
            <div className="flex flex-wrap gap-3.5">
              {projects.map((project) => (
                // FIXED: Used backticks for navigation
                <div onClick={() => navigate(`/project/${project.id}`)} key={project.id}
                  ...
                  <div className="p-4 text-white bg-gradient-to-b from-transparent group-hover:from-indigo-950/20 transition-colors">
                    ...
                    <div onClick={(e) => e.stopPropagation()} className="flex justify-between items-center mt-6">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => navigate(`/preview/${project.id}`)} className='px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md text-xs transition-all'>Preview</button>
                        <button onClick={() => navigate(`/project/${project.id}`)} className="px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md text-xs transition-colors">Open</button>
                      </div>
                    </div>
                  </div>

                  <div onClick={e => e.stopPropagation()}>
                    <TrashIcon className='absolute top-3 right-3 opacity-0 group-hover:opacity-100 bg-white p-1.5 size-7 rounded text-red-500 cursor-pointer transition-all'
          ...
          <div className='flex flex-col items-center justify-center h-[80vh]'>
            <h1 className='text-xl font-semibold text-gray-300'>You have no projects yet.</h1>
            <button onClick={() => navigate('/')} className="mt-5 px-5 py-2 bg-indigo-500 text-white rounded-md">Create New</button>
          </div>
        )}
      </div>
      <Footer />
